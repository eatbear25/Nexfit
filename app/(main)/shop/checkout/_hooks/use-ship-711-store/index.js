...
function CallbackContent({ keyLocalStorage }) {
  // eslint-disable-next-line
  const [storedValue, setValue] = useLocalStorage(keyLocalStorage, {
    storeid: "",
    storename: "",
...
export function useShip711StoreCallback(keyLocalStorage = "store711") {
  ...
  useEffect(() => {
    ...
    const handleCallback = () => {
      ...
      // 設定到localStorage
      localStorage.setItem(keyLocalStorage, JSON.stringify(paramsObj));
  ...
  // 為了相容性，也提供 Suspense 版本的組件
  return {
    CallbackComponent: () => (
      <CallbackContent keyLocalStorage={keyLocalStorage} />
    ),
  };
}
